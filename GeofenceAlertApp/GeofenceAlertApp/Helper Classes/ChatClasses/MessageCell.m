...
//    _textView.font = [UIFont fontWithName:@"Helvetica" size:17.0];
    _textView.font = [UIFont fontWithName:CGRegular size:textSize+2];
    if (IS_IPHONE)
    {
        _textView.font = [UIFont fontWithName:CGRegular size:textSize];
    }
...
//        textView_x -= [self isStatusFailedCase]?65.0:0.0;
    }
    else
    {
        textView_x = 20;
...
- (void)setBubble
{
    ...
    CGFloat bubble_height = MIN(_textView.frame.size.height + 8,
                                _timeLabel.frame.origin.y + _timeLabel.frame.size.height + fixedHeight);
...
-(void)setStatusIcon
{
    ...
    if (self.message.status == MessageStatusFailed)
        _statusIcon.image = [self imageNamed:@"status_failed"];
